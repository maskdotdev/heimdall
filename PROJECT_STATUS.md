...
| #23 Static analysis integration | Partial | `packages/static-analysis`, `packages/tool-runner`, `packages/review-engine` | `@repo/static-analysis` now exposes typed tool descriptors, changed-file-fast planning, tool budgets, deterministic diagnostic normalization, diff-line mapping, ESLint JSON output parsing, and report building through a runner boundary. `@repo/tool-runner` provides the typed command-runner contract, deterministic fake runner, and a local shell-free process runner with timeout and output budgets. `@repo/review-engine` can now synthesize candidate findings from changed-line static diagnostics. Sandbox-backed execution, broader live tool output parsers, raw output storage, orchestrator wiring, and dashboard/debug API surfaces remain. |
