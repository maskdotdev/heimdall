...
- Latest DB sandbox milestone: `SandboxRepository` now owns stale sandbox run cleanup target
  selection, sandbox artifact URI lookup for backing-file cleanup, and sandbox run deletion with
  database cascades for child rows. The worker uses this boundary for `sandbox.cleanup.v1` jobs,
  and the Postgres-backed integration test verifies ordered repo-scoped selection, limit
  validation, URI reads, and cascaded child deletion.
