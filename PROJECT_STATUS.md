...
- Latest index artifact schema milestone: `@repo/index-schema` now owns artifact validation
  ...
  helpers, whole-artifact JSON helpers, Node split-artifact reader/writer helpers, partitioned
  record-file reads with count/byte/SHA-256 verification, record-file path safety checks, and
  artifact diff utilities. The indexer driver re-exports the schema-owned validator for
