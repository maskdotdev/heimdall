...
- Latest DB billing milestone: `BillingRepository` now owns outbound provider request audit
  logging and failed provider-request listing. The API and worker use this boundary for Stripe
  provider request audit logging, the API uses it for billing reconciliation issue reads, and the
  Postgres-backed integration test verifies idempotent request-key updates, org-scoped failed
  request ordering, and bounded list validation.
