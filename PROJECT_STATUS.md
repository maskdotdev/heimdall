...
- Latest DB provider-installation milestone: `ProviderInstallationRepository` now owns provider
  ...
  `orgs` or `provider_installations` tables, and the Postgres-backed repository integration test
  verifies conflict-safe org and installation updates.
- Latest DB organization milestone: `OrganizationRepository` now owns organization list/detail
  summary reads with installation and repository aggregate counts. The API uses this repository
...
- Latest production artifact deployment milestone: the Railway production manifest and deployment
  ...
  worker services, SecretRef names for GitHub webhook/private-key and provider API keys, and at
  least one production SecretRef provider environment group for AWS, GCP, or Vault secret lookup.
  The manifest also declares, and the audit validates, that production review artifact storage uses
  a private S3-compatible bucket with public access blocked, provider-managed encryption, and
  support-session-gated raw downloads.
...
- Latest sandbox milestone: Phase #24 now has a reusable sandbox execution contract package.
  ...
  `pnpm proof:sandbox:staging` now provides a non-mutating staging evidence gate that logs in
  through the configured admin identity gateway, reads `/admin/sandbox/runs`, and validates
  deployed HTTPS targets, requested org/repo/review scope, non-`local_process` runners, successful
  status, zero denied policy decisions, untruncated outputs, and collected artifact hashes before
  writing a product-safe proof record.
...
- Latest observability milestone: Phase #25 now includes typed runtime configuration and resource
  ...
  health responses, with readiness covering runtime configuration, GitHub App configuration, and
  Postgres and Redis availability by default while allowing deterministic injected checks in tests
  and custom composition. The
  ...
  response and latest worker queue-health samples, and the operator dashboard renders each
  readiness check plus queue backlog age as runtime health. OTLP exporter mode now initializes
  OpenTelemetry log, metric, and trace providers with product-safe facades,
...
- Latest observability service-coverage milestone: the admin gateway now depends on
  ...
  now use structured telemetry instead of direct console writes. The indexer CLI now initializes
  the shared runtime with a stable `heimdall-indexer-cli` service identity, routes console
  telemetry to stderr so artifact JSON remains parseable on stdout, and wraps local TypeScript
  indexing in `withIndexerTelemetry` so CLI-driven indexes emit the same product-safe indexer run
  metrics and spans as worker-driven indexing.
- Latest observability tracking milestone: Phase #25 is now tracked as done for the MVP
  instrumentation scope because API, admin-gateway, worker, GitHub provider, repo-sync,
  indexer-driver, index-importer, embedding, static-analysis, sandbox, memory/feedback, retrieval,
  LLM, publisher, dashboards, alerts, local OTLP stack wiring, structured logs, and redaction tests
  all have product-safe telemetry coverage.
...
- Latest security/compliance milestone: Phase #27 now has data classification, retention, and
  ...
  resolution, AWS Secrets Manager resolution, GCP Secret Manager resolution, Vault KV resolution, production-provider
  placeholders, redacted resolved-secret helpers, and rotation record contracts. The worker resolves
  its GitHub App private key through that boundary using
...
- Latest uninstall deletion execution milestone: GitHub installation removal webhooks now persist
  ...
  metadata.
- Latest provider artifact deletion milestone: `@repo/github` now exposes provider cleanup
  methods for deleting issue comments, deleting pull-request review comments, and redacting check
  run output for data deletion. `DataDeletionRepository` can list provider-visible GitHub
  publication targets for scoped repositories, and worker deletion execution now attempts that
  cleanup for installed repository and organization deletion scopes before completing the request.
  App-uninstall-triggered deletion still skips remote cleanup because the installation token is no
  longer available after GitHub removes the installation.
- Latest worker security-event milestone: the worker runtime now creates a Postgres-backed
  ...
  families when GitHub clone-auth acquisition fails before repo-sync checkout. Scheduled review
  thread reconciliation now records those provider-control event families when GitHub thread-state
  fetches fail before provider feedback updates.
...
- Latest review/provider security-event milestone: review orchestration now accepts the worker
  ...
  checks, including staleness checkpoints that degrade to `unknown` instead of failing the review.
  Worker review jobs pass through the durable Postgres-backed sink, and focused
  review-orchestrator tests verify permission-denial event shape, safe rate-limit metadata,
  checkpoint metadata, and not-found suppression for stale PR state.
- Latest publisher security-event milestone: `@repo/publisher` now accepts the worker
  ...
  Worker publish jobs pass through the durable Postgres-backed sink, and focused publisher tests
  verify permission-denial event shape plus owner, repository-name, and token redaction.
