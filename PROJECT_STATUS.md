...
| #17 LLM gateway | Partial | `packages/llm-gateway`, `packages/review-orchestrator/src/index.ts`, `packages/db/src/schema/tables.ts`, `apps/worker/src/index.ts`, `packages/admin-tools/src/live-openai-llm-smoke.ts`, `docs/runbooks/llm-provider-smoke.md` | Schema-validating structured-output gateway, deterministic static adapter, fixture-backed fake provider, normalized gateway error model, bounded retry policy, and an OpenAI-compatible Chat Completions provider for JSON-mode structured review findings exist. The gateway now redacts secret-like prompt content before provider calls by default, and review orchestration records successful model-call rows from the same redacted prompt text with provider/model, prompt/response hashes, token estimates, latency, rate-card metadata, and cost. The worker can select the real provider with `LLM_PROVIDER=openai` plus `OPENAI_MODEL`/`LLM_MODEL` and SecretRef-backed API-key configuration while keeping the smoke gateway as explicit `HEIMDALL_REVIEW_SMOKE_FINDING=true` mode. Admin tooling now includes a guarded OpenAI-compatible live LLM smoke runner and runbook that exercise schema-valid review findings without printing API keys, prompts, raw responses, or customer data. Prompt/version management, budget enforcement, redacted LLM artifacts, and recorded live model smoke evidence remain. |
...
- Latest LLM gateway provider milestone: `@repo/llm-gateway` now normalizes provider and schema
  ...
  before the existing TypeBox validation layer. The worker can now construct that gateway from
  environment settings and SecretRef-backed API-key resolution. Admin tooling now includes a
  guarded OpenAI-compatible live LLM smoke runner plus a runbook that prints product-safe proof
  without exposing API keys, prompts, raw responses, or customer data.
