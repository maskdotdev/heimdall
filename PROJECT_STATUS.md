...
- Latest DB repository milestone: `RepositoryRepository` now supports provider-native repository
  lookup and deterministic enabled-repository cursor pagination. The new Postgres-backed
  integration test applies the current migration chain in an isolated schema and verifies provider
  lookup, enabled-only filtering, org scoping, cursor advancement, and cursor/limit validation.
...
- Latest DB code-intelligence milestone: `CodeIntelligenceRepository` now maps imported symbol,
  chunk, and edge rows back to index-record contracts and supports symbol-at-line, file symbol,
  file chunk, outgoing edge, and incoming edge lookups. The Postgres-backed integration test
  verifies latest-ready symbol selection, innermost line matching, source ordering, chunk metadata
  mapping, and edge kind filtering.
