...
- Latest review orchestrator state-machine milestone: review orchestration now persists
  ...
  current failed stage when a run fails mid-flow. It now performs provider current-head checks
  after snapshot, after index wait, before review/model execution, and before publish handoff,
  records each staleness checkpoint result, releases reserved quota for stale terminal outcomes,
  and marks moved or closed PRs as superseded or skipped before later expensive work or publisher
  jobs are enqueued.
