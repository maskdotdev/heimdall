...
| #13 Embedding pipeline | Partial | `packages/embedding`, `apps/worker/src/index.ts`, `packages/embedding/test` | Durable embedding jobs are handled by the worker, deterministic 1536-d embeddings are stored in pgvector, vector dimensions are validated, progress is cumulative, code chunk embedding inputs now include stable metadata headers, SHA-256 input hashes, conservative token estimates, truncation, provider request batching, an OpenAI-compatible Embeddings adapter, and worker environment selection with SecretRef-backed API-key resolution. Usage/cost records, durable cache reuse, embedding job tables, live provider smoke evidence, and broader telemetry remain. |
...
- Latest embedding input milestone: `@repo/embedding` now builds `code_chunk` provider inputs with
  ...
  SecretRef-backed API-key resolution.
