...
- Latest security/compliance milestone: Phase #27 now has data classification, retention, and
  ...
  `GITHUB_APP_PRIVATE_KEY_SECRET_REF` with local env fallback. It now also resolves LLM provider
  API keys through `LLM_PROVIDER_API_KEY_SECRET_REF`, `OPENAI_API_KEY_SECRET_REF`, or local
  environment fallbacks before constructing the real OpenAI-compatible review gateway. The API
