...
| #21 Feedback and memory system | Partial | `packages/memory`, `packages/memory/test/memory.test.ts`, `packages/retrieval`, `packages/db/src/schema/tables.ts`, `packages/admin-tools/src/index.ts`, `apps/web/src/main.ts` | Package now exposes typed feedback events, signals, outcome states, memory candidates/facts, hidden marker parsing, deterministic command parsing, outcome scoring, memory candidate activation, relevant memory lookup with trace/budgets, retrieval context integration, and an explainable in-memory suppression matcher. Durable `memory_facts` and `memory_candidates` tables now provide the persistence foundation for configured memory and candidate moderation, and the memory/rules inspector surfaces applicable candidate rows next to stored memory facts. Webhook job integration, candidate approval/rejection workflows, dashboard moderation actions, and broader feedback-to-memory automation remain. |
