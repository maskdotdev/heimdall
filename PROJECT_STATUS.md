...
| #15 PR snapshot and diff model | Partial | `packages/pr-snapshot`, `packages/webhook-ingestion/src/github/payload.ts`, `packages/github`, `packages/db/src/repositories/pull-request-repository.ts`, `408f7bd` | Webhook payload normalization creates shallow snapshots, the GitHub provider can fetch full changed-file snapshots plus raw diff hashes, review orchestration refreshes persisted snapshots for a fetched head SHA, and `@repo/pr-snapshot` now parses basic unified diffs into provider-neutral changed-file contracts with raw diff hashing and commentable line indexes. GitHub anchor conversion, multiline anchors, persistence helpers, and broader golden fixtures remain. |
...
- Latest PR snapshot milestone: `@repo/pr-snapshot` now provides the first provider-neutral unified
  diff parser, raw diff hashing, minimal patch reconstruction, and left/right commentable line
  indexing for parsed changed-file contracts. Tests cover modified, renamed, binary, and
  commentability cases.
