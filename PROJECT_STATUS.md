...
| #8 Repo sync and workspace manager | Partial | `packages/repo-sync`, `apps/worker/src/index.ts`, `packages/review-orchestrator/src/index.ts` | Repo sync can obtain GitHub clone auth, validate lowercase full-length commit SHAs, sanitize and allowlist clone URLs, run Git through a timeout-aware runner with redacted failures, define cache configuration/layout path builders, create/reuse atomic bare mirrors, verify exact commits in mirrors through ref-hint/direct fetches, create detached worktree leases, acquire cached exact-commit workspaces, create an exact-commit workspace, verify `HEAD`, hand the workspace to the TypeScript indexer, expose repository path safety helpers, clean up temporary workspaces through a guarded cleanup helper that refuses relative, unmanaged, or outside-root paths before deleting retained workspaces, and remove expired cached worktrees after process crashes. Worker index jobs and review orchestration now use cached acquisition paths, and worker startup runs expired-worktree cleanup. |
