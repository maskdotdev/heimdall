...
| #7 Job queue and orchestration | Partial | `packages/queue`, `apps/worker`, `apps/worker/src/index.test.ts`, `packages/db/src/schema/tables.ts`, `packages/db/src/repositories/background-job-repository.ts`, `packages/admin-tools/src/index.ts`, `apps/api/src/app.ts`, `apps/web/src/main.ts` | Current async backbone scope exists: pending durable rows, outbox dispatch to BullMQ, worker lifecycle updates, retry/idempotency coverage, stale running job recovery, environment-selected worker role queues for independent scaling, worker handler registration for indexing, embedding, review, publishing, and billing reconciliation jobs, plus audited admin retry/replay/cancel controls. Heartbeat-backed active cancellation checkpoints and broader queue health controls remain. |
