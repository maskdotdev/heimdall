...
| #11 TypeScript indexer | Partial | `packages/indexer-ts`, `packages/indexer-ts/test` | TypeScript indexer emits files, symbols, edges, chunks, manifest hashes, deterministic fixtures, generated/vendor skip heuristics for path and header patterns, and symlink skipping so workspace walks do not follow linked source files outside the checkout. Broader language coverage and incremental behavior remain. |
...
- Latest TypeScript indexer milestone: `@repo/indexer-ts` now skips generated and vendored source
  ...
  outside the checkout through linked `*.ts`/`*.js` paths.
