...
| #26 Evaluation harness | Partial | `packages/evaluation`, `packages/evaluation/fixtures/smoke-full-pipeline-v1.json`, `pnpm eval:ci`, `packages/db/src/schema/tables.ts`, `packages/contracts/src/api/evaluation.ts`, `apps/api/src/app.ts`, `apps/web/src/main.ts` | MVP deterministic suite runner, 12 curated cases, exact finding matching, anchor grading, retrieval recall, latency/cost metrics, Markdown/JSON/JUnit/HTML reports, CI threshold gate, baseline metric deltas, case-level comparison rows, lost/new true positive detection, new/resolved false positive detection, CI-safe comparison Markdown, eval history DB schema, eval history repository writes/queries, optional CLI history persistence, reviewed production-case import helpers, read-only admin eval history API routes, and an operator dashboard view for suites, runs, and case results exist. Broader live-model suites, human labeling workflows, and scheduled persistence wiring remain. |
...
- Latest evaluation milestone: Phase #26 baseline comparison now includes case-level comparison
  ...
  protection and an explicit replacement flag.
