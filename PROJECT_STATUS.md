...
| #1 Monorepo and build system | Partial | `package.json`, `pnpm-workspace.yaml`, `turbo.json`, `tsconfig.json`, `.github/workflows/ci.yml`, `test/workspace-tsconfig.test.ts` | Workspace, TypeScript, Biome, Vitest, Turbo, boundary checks, and a GitHub Actions CI gate are active. Root TypeScript project references now cover every workspace project with a guard test to prevent drift. Broader release automation remains. |
...
| #12 Index importer | Partial | `packages/index-importer`, `packages/db`, `apps/worker/src/index.ts` | Importer persists index versions, files, symbols, edges, chunks, diagnostics, dependencies, routes, test mappings, and durable embedding jobs idempotently, now includes filesystem/file-URI and S3/R2-compatible whole-artifact resolvers, an S3/R2-compatible whole-artifact store for URI handoffs, plus filesystem split-layout resolution for `index-manifest.json` and streamed `records.jsonl`, `importIndexArtifactFromUri` for URI-first handoff paths, the worker uses that URI-first path for locally persisted and object-storage-uploaded index artifacts, S3/R2-compatible object-storage mode can server-side copy remote indexer artifact objects into the configured Heimdall bucket before import, normalized record plus embedding planner item writes are bounded into configurable insert batches, normalized record writes now flow through an internal Drizzle batch writer abstraction, worker imports can tune those insert batches with `HEIMDALL_INDEX_IMPORT_RECORD_BATCH_SIZE` or `INDEX_IMPORT_RECORD_BATCH_SIZE`, import attempts now write durable `index_import_batches` phase/outcome rows, and configurable import limits reject oversized artifacts before DB record writes while split JSONL readers enforce line/count limits during streaming reads. COPY paths, database-level streaming import, and broader bulk object paths remain. |
