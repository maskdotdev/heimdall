...
| #31 Testing and evaluation strategy | Partial | `pnpm check`, `pnpm eval:ci`, `pnpm ci:control-plane:release`, `.github/workflows/ci.yml`, `.github/workflows/evaluation-history.yml`, `test/ci-workflow.test.ts`, `test/package-test-coverage.test.ts`, `packages/evaluation/test/evaluation.test.ts`, package tests | Unit tests, package test coverage guards, Postgres-backed migration/review integration tests in CI, release gates, fake PR review integration coverage, the deterministic evaluation gate, baseline comparison CLI regression coverage, scheduled eval-history workflow wiring, and CI upload of deterministic eval Markdown/JSON reports now run for new work. Root workflow tests guard the release gate script path, report artifacts, pgvector test database wiring, scheduled eval-history persistence flags, fake GitHub/static-LLM PR review coverage, and representative redaction/cross-tenant tests. Evaluation package tests run the real Bun CLI against a regressed candidate report and verify both default failure and report-only output. Remaining gaps are provider-configured production eval history evidence and broader release-hardening coverage. |
