...
- Latest DB review milestone: `ReviewRepository` now owns provider-feedback target lookups for
  ...
  worker uses this boundary for provider-feedback, thread-reconciliation, and index-dependency
  review-run lookup paths, and the Postgres-backed integration test verifies ordered
  provider-comment target lookup plus pull-request-filtered recent run and waiting-for-index
  listing.
