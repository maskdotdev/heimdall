...
- Latest security data-deletion milestone: `@repo/contracts` now defines durable data-deletion
  ...
  idempotent request creation, scoped listing, lookup, and lifecycle transitions. The guarded
  Postgres integration test verifies request idempotency, org/repo/status filtering, newest-first
  ordering, completion evidence updates, and limit validation. Worker/API deletion execution across
  object storage, embeddings, queued jobs, and provider records remains the next security step.
- Latest uninstall deletion-planning milestone: GitHub installation removal webhooks now persist
  ...
  needed, and transition it to `planned` with an initial product-safe manifest for the organization,
  repository, user, review-run, or artifact-class scope.
