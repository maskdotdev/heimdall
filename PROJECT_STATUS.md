...
| #13 Embedding pipeline | Partial | `packages/embedding`, `apps/worker/src/index.ts`, `packages/db/src/schema/tables.ts`, `packages/embedding/test` | Durable embedding jobs are handled by the worker, deterministic 1536-d embeddings are stored in pgvector, vector dimensions are validated, progress is cumulative, code chunk embedding inputs now include stable metadata headers, SHA-256 input hashes, conservative token estimates, truncation, provider request batching, an OpenAI-compatible Embeddings adapter, worker environment selection with SecretRef-backed API-key resolution, cache-key metadata columns, durable vector reuse by provider/model/dimension/input hash/profile, and idempotent `embedding.token` usage events with estimated provider-token cost metadata for cache-miss provider batches. Embedding job tables, live provider smoke evidence, exact provider-returned token ingestion, and broader telemetry remain. |
...
- Latest embedding input milestone: `@repo/embedding` now builds `code_chunk` provider inputs with
  ...
  input-count, estimated token, and rate-card cost metadata.
