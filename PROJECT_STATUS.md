...
- Latest repo-local rule action milestone: `@repo/rules` now compiles repo-local
  `enabled_categories`, `disabled_categories`, and `severity_threshold` rule actions into
  source-attributed synthetic suppression rules. Minimum-confidence actions emit an explicit
  warning until confidence-aware rule matching exists.
