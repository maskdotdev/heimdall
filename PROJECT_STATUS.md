...
| #12 Index importer | Partial | `packages/index-importer`, `apps/worker/src/index.ts` | Importer persists index versions, files, symbols, edges, chunks, and durable embedding jobs idempotently, now includes filesystem/file-URI and S3/R2-compatible whole-artifact resolvers plus filesystem split-layout resolution for `manifest.json` and `records.jsonl`, `importIndexArtifactFromUri` for URI-first handoff paths, the worker uses that URI-first path for locally persisted index artifacts, and normalized record writes are bounded into configurable insert batches. COPY paths, richer import batch state, streaming record import, and object-copy/bulk object paths remain. |
...
- Latest index importer URI milestone: `@repo/index-importer` now exposes filesystem/file-URI and
  ...
  artifacts. Normalized file, symbol, edge, and chunk rows are now written in bounded configurable
  insert batches so large artifacts avoid one unbounded insert per record type.
