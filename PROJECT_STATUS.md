...
- Latest embedding input milestone: `@repo/embedding` now builds `code_chunk` provider inputs with
  ...
  job counters. Index import schedules a delayed `embedding.repair.v1` backstop for each planned
  embedding job, and the worker handles both single-job and scoped repair payloads.
