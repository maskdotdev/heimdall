...
| #14 Retrieval engine | Partial | `packages/retrieval`, `packages/review-orchestrator/src/index.ts`, `packages/retrieval/test/retrieval.test.ts` | Retrieval now uses imported index rows for same-file, symbol, graph, exact test mappings with filename fallback, changed-manifest dependency context, changed-file route/config context, PostgreSQL full-text, and vector-backed context with diff fallback, validates returned context bundles against the shared TypeBox contract, and records non-fatal warnings when optional indexed retrievers fail. Repo rules and dashboard inspection remain. |
