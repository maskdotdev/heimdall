...
| #7 Job queue and orchestration | Partial | `packages/queue`, `apps/worker`, `packages/db/src/schema/tables.ts`, `packages/db/src/repositories/background-job-repository.ts`, `packages/admin-tools/src/index.ts`, `apps/api/src/app.ts`, `apps/web/src/main.ts` | Current async backbone scope exists: pending durable rows, outbox dispatch to BullMQ, worker lifecycle updates, retry/idempotency coverage, stale running job recovery, worker handler registration for indexing, embedding, review, publishing, and billing reconciliation jobs, plus audited admin retry/replay/cancel controls. Independent worker-role scaling, heartbeat-backed active cancellation checkpoints, and broader queue health controls remain. |
