...
- Latest LLM gateway provider milestone: `@repo/llm-gateway` now normalizes provider and schema
  ...
  the worker can configure those budgets from environment settings. The worker can now construct
  that gateway from environment settings and SecretRef-backed API-key resolution. Admin tooling now
  includes a guarded OpenAI-compatible live LLM smoke runner plus a runbook that prints product-safe
  proof without exposing API keys, prompts, raw responses, or customer data.
...
- Latest embedding input milestone: `@repo/embedding` now builds `code_chunk` provider inputs with
  ...
  handles both single-job and scoped repair payloads. Admin tooling now includes a guarded live
  OpenAI-compatible embedding smoke runner plus a runbook that prints product-safe proof without
  exposing API keys or vector contents.
