...
- Latest LLM gateway provider milestone: `@repo/llm-gateway` now normalizes provider and schema
  ...
  before the existing TypeBox validation layer. Review-finding calls now carry a stable prompt
  version, the gateway can reject over-budget prompt/input payloads before provider execution, and
  review orchestration persists linked redacted LLM prompt/response artifacts for replay/debug
  inspection. The worker can configure those budgets from environment settings and construct that
  gateway with SecretRef-backed API-key resolution. Admin tooling now includes a guarded
...
- Latest embedding input milestone: `@repo/embedding` now builds `code_chunk` provider inputs with
  ...
  stale scope drift. Admin tooling now includes a guarded live OpenAI-compatible embedding smoke
  runner plus a runbook that prints product-safe proof without exposing API keys or vector contents.
