...
- Latest embedding input milestone: `@repo/embedding` now builds `code_chunk` provider inputs with
  ...
  input-count, estimated token, and rate-card cost metadata. Index import now also creates
  ...
  and parent progress counters as batches run, skip, succeed, or fail.
