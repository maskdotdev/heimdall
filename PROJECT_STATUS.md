...
| #19 Finding validation, dedupe, and ranking | Partial | `packages/review-engine`, `packages/review-orchestrator/src/index.ts`, `packages/db` | Candidate findings now flow through deterministic anchor validation, policy-derived severity/category/confidence gates, basic repo-rule suppression, durable memory suppression, exact, location, and conservative semantic duplicate suppression, budget limiting, and ranking before persistence. `@repo/review-engine` also exposes an inspectable validation result with accepted/rejected findings, duplicate groups, rejection stats, and product-safe validation trace events while preserving the existing `ValidatedFinding[]` path. Review orchestration now loads active repository and organization memory facts into validation, persists durable validation-event rows, and persists rejected-findings plus validation ranking-report artifacts that include validation stats, duplicate groups, and trace events. Publish-plan output remains. |
...
- Latest finding validation milestone: `@repo/review-engine` now exposes `validateCandidateFindings`
  ...
  reason, and validator metadata for replay/debug inspection.
