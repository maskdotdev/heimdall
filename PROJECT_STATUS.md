...
- Latest index artifact schema milestone: `@repo/index-schema` now owns artifact validation
  ...
  validate, print-manifest, count-records, and diff. The indexer driver re-exports the schema-owned
  validator for compatibility, while the indexer CLI and importer use the shared helpers instead of
  duplicating split-layout and JSONL handling.
