...
| #8 Repo sync and workspace manager | Partial | `packages/repo-sync`, `apps/worker/src/index.ts`, `packages/review-orchestrator/src/index.ts` | Repo sync can obtain GitHub clone auth, validate lowercase full-length commit SHAs, sanitize and allowlist clone URLs, run Git through a timeout-aware runner with redacted failures, define cache configuration/layout path builders, serialize mirror/fetch/worktree cache mutations with bounded filesystem locks, create/reuse atomic bare mirrors, verify exact commits in mirrors through ref-hint/direct fetches, create detached worktree leases, persist local sidecar lease metadata, verify leased worktree `HEAD` before returning cached workspaces, enforce max workspace bytes before returning leases, report local cache stats for mirrors/worktrees/metadata, acquire cached exact-commit workspaces, create an exact-commit workspace, verify `HEAD`, hand the workspace to the TypeScript indexer, expose repository path safety helpers, clean up temporary workspaces through a guarded cleanup helper that refuses relative, unmanaged, or outside-root paths before deleting retained workspaces, and remove expired cached worktrees after process crashes. Worker index jobs and review orchestration now use cached acquisition paths, and worker startup runs expired-worktree cleanup. |
...
- Latest repo-sync worktree-validation milestone: cached worktree leases now verify `git rev-parse
  HEAD` after worktree creation and before returning the lease. If Git resolves a different commit,
  repo-sync removes the failed lease path and prunes worktree metadata instead of handing an
  unverified workspace to index or review jobs.
...
- Latest worker repo-sync integration milestone: worker index jobs now acquire repository
  ...
  driver, and releases the worktree lease after index artifact import.
- Latest review repo-sync integration milestone: review orchestration now uses a cached repo-sync
  ...
  retains leases only for static-analysis execution, and releases cached worktree leases through the
  lease API instead of deleting worktree paths directly.
