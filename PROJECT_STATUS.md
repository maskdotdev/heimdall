...
| #15 PR snapshot and diff model | Partial | `packages/pr-snapshot`, `packages/webhook-ingestion/src/github/payload.ts`, `packages/github`, `packages/db/src/repositories/pull-request-repository.ts`, `408f7bd` | Webhook payload normalization creates shallow snapshots, the GitHub provider can fetch full changed-file snapshots plus raw diff hashes, review orchestration refreshes persisted snapshots for a fetched head SHA, and `@repo/pr-snapshot` now parses basic unified diffs into provider-neutral changed-file contracts with raw diff hashing, commentable line indexes, GitHub review-comment anchor conversion for verified same-side anchors, and deterministic change-set extraction for ranges, modified blocks, path sets, and renames. Broader multiline policies, persistence helpers, and broader golden fixtures remain. |
...
- Latest PR snapshot milestone: `@repo/pr-snapshot` now provides the first provider-neutral unified
  diff parser, raw diff hashing, minimal patch reconstruction, left/right commentable line indexing,
  and GitHub review-comment anchor conversion for verified single-line and same-side multiline
  anchors. It also extracts a contract-validated change set with added/deleted ranges, conservative
  modified blocks, changed path sets, and rename pairs for retrieval handoff. Tests cover modified,
  renamed, binary, commentability, unsafe-anchor, and change-set cases.
