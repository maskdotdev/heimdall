...
- Latest compliance evidence milestone: `@repo/security` now defines stable MVP compliance control
  ...
  Completion and failure outcomes for those jobs now emit product-safe worker security events.
  Collector tests verify that raw audit metadata and custom repository instructions are excluded
  from exported evidence, and worker tests verify dispatch plus failure event emission through the
  configured collector hook.
