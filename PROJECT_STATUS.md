...
- Latest PR snapshot milestone: `@repo/pr-snapshot` now provides the first provider-neutral unified
  ...
  left/right commentable line indexing, file-level fallback indexing for binary/rename-only/metadata
  changes, and GitHub review-comment anchor conversion for verified single-line, same-side
  multiline, and file-level anchors. It also extracts a contract-validated change set with
  added/deleted ranges, conservative modified blocks, changed path sets, and rename pairs for
  retrieval handoff. Tests cover modified, renamed, binary, hashing, commentability, unsafe-anchor,
  file-anchor, and change-set cases.
