...
- Latest DB repository milestone: `RepositoryRepository` now supports provider-native repository
  lookup, owning-org lookup, deterministic enabled-repository cursor pagination, batch
  repository/settings/org-default reads, provider metadata upserts that preserve product
  enablement, and insert-if-absent default repository settings. GitHub webhook ingestion uses this
  repository boundary for repository planning state and repository/settings persistence, and
  `@repo/embedding` uses it for usage-ledger org scope lookup. The Postgres-backed integration
  test applies the current migration chain in an isolated schema and verifies provider lookup,
  owning-org lookup, enabled-only filtering, org scoping, cursor advancement, cursor/limit
  validation, enablement preservation, insert-if-absent settings behavior, and batch settings
  lookups.
