...
- Latest dashboard route-state milestone: the dashboard now parses and writes URL query state for
  ...
  evaluation selection, product organization, product repository, review run, and selected finding.
  The same state is applied after product session and admin session reloads so major dashboard
  views are shareable and reload-safe.
