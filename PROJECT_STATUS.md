...
| #21 Feedback and memory system | Partial | `packages/memory`, `packages/memory/test/memory.test.ts`, `packages/retrieval`, `packages/db/src/schema/tables.ts`, `packages/admin-tools/src/index.ts`, `apps/api/src/app.ts`, `apps/web/src/main.ts` | Package now exposes typed feedback events, signals, outcome states, memory candidates/facts, hidden marker parsing, deterministic command parsing, outcome scoring, memory candidate activation, relevant memory lookup with trace/budgets, retrieval context integration, and an explainable in-memory suppression matcher. Durable `memory_facts` and `memory_candidates` tables now provide the persistence foundation for configured memory and candidate moderation, the memory/rules inspector surfaces applicable candidate rows next to stored memory facts, and scoped API moderation endpoints can approve candidates into audited durable memory facts or reject them after `memory:write` authorization. Webhook job integration, dashboard moderation actions, and broader feedback-to-memory automation remain. |
...
- Latest admin tooling milestone: review inspectors can now export redacted debug bundles through
  ...
  repository/org rules, stored memory facts, and policy-tool availability; the scoped API can now
  approve persisted memory candidates into durable memory facts or reject them with audit logs.
  Dashboard moderation actions remain to wire. PR12 hardening now includes cross-tenant API tests
  that verify debug bundle export, eval import draft creation, and Memory & Rules inspection are
  denied before sensitive actions run, plus configurable API-side
...
- Latest memory inspector milestone: admin and web memory/rules inspection now list applicable
  memory candidates with status, trust, source, confidence, scope/applicability keys, and updated
  moderation warnings.
- Latest memory moderation milestone: `/api/v1/memory-candidates/:memoryCandidateId/approve` and
  ...
  idempotency keys, write audit logs, and create or reuse durable memory facts on approval.
