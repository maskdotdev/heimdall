...
- Latest DB security-audit milestone: `SecurityAuditRepository` now owns artifact access event
  writes, idempotent normalized security event writes, and audit log insertion. The API uses this
  repository boundary for sensitive review-artifact access logging, security event sink writes, and
  admin audit writes, with Postgres-backed integration coverage for duplicate security-event
  handling and inserted artifact/audit rows.
