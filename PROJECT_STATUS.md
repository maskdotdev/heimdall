...
- Latest DB background-job milestone: `BackgroundJobRepository` now supports idempotent durable
  ...
  repository boundary, and the Postgres-backed integration test verifies insert idempotency,
  scheduled pending filtering, running/completed/missing lifecycle states, retry error storage, and
  stale retry/dead-letter repair.
