...
- Latest DB repository milestone: `RepositoryRepository` now supports provider-native repository
  lookup, deterministic enabled-repository cursor pagination, batch repository/settings/org-default
  reads, provider metadata upserts that preserve product enablement, and insert-if-absent default
  repository settings. GitHub webhook ingestion now uses this repository boundary for repository
  planning state and repository/settings persistence. The Postgres-backed integration test applies
  the current migration chain in an isolated schema and verifies provider lookup, enabled-only
  filtering, org scoping, cursor advancement, cursor/limit validation, enablement preservation,
  insert-if-absent settings behavior, and batch settings lookups.
...
- Latest DB embedding milestone: `EmbeddingRepository` now exposes embeddable chunk reads, reusable
  ...
  pgvector similarity search. Retrieval semantic search now calls this repository boundary, and the
  Postgres-backed integration test verifies metadata mapping, conflict-safe vector writes, progress
  updates, cache/content-hash reuse, and nearest-neighbor ordering.
