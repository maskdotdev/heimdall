...
- Latest DB background-job milestone: `BackgroundJobRepository` now supports idempotent durable
  ...
  stale-running recovery, plus embedding-job cleanup for failed index imports. `@repo/queue` now
  ...
  jobs, and failed-import job cleanup, and the Postgres-backed integration test verifies insert
