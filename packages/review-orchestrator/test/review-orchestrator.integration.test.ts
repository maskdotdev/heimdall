  ...
  it("persists a review run, findings, validation output, and publish job", async () => {
    ...
    const [counts] = await sql`
      SELECT
        ...
        (SELECT count(*)::int FROM review_run_stage_events WHERE stage = 'gate' AND status = 'completed') AS gate_events,
    ...
    expect(counts).toEqual({
      ...
      code_redaction_artifacts: 11,
      consumed_quota_reservations: 1,
      customer_code_artifacts: 15,
      customer_confidential_artifacts: 4,
      duplicate_groups: 1,
      gate_events: 1,
      line_anchor_artifacts: 1,
      llm_call_artifact_links: 4,
      llm_prompt_artifacts: 2,
      llm_response_artifacts: 2,
